...
class ZstdFrameAutoDecoder(
    ...
    override fun decode(ctx: ChannelHandlerContext, input: ByteBuf, out: MutableList<Any>) {
        ...
        if (!mode) {
            if (input.isReadable) out += input.readRetainedSlice(input.readableBytes())
            return
        }
...
class ZstdFrameEncoder(
    ...
    override fun encode(ctx: ChannelHandlerContext, msg: ByteBuf, out: ByteBuf) {
        ...
        if (length == 0) return
        if (onlyWhenDetected && !ctx.channel().isZstdFrameEnabled()) {
...
private fun decodeZstdFrame(
    ...
    if (uncompressedLength < 0 || payloadLength < 0 ||
        uncompressedLength > maxFrameSize || payloadLength > maxFrameSize
    ) {
        throw CorruptedFrameException(
            "invalid zstd frame size raw=$uncompressedLength payload=$payloadLength"
        )
    }
    ...
    if ((flags and FLAG_COMPRESSED) == 0) {
        if (payloadLength != uncompressedLength) {
            throw CorruptedFrameException(
                "raw zstd frame size mismatch: $payloadLength != $uncompressedLength"
            )
        }
        out += ctx.alloc().buffer(payload.size).writeBytes(payload)
        return
    }
    ...
    if (decompressed.size != uncompressedLength) {
        throw CorruptedFrameException(
            "zstd decoded size mismatch: ${decompressed.size} != $uncompressedLength"
        )
    }
