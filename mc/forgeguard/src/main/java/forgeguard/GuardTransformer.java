...
public final class GuardTransformer implements ClassFileTransformer {
    private static final String HOOKS = "forgeguard/GuardHooks";
    ...
    private static final boolean PATCH_GTO_MIXIN_PLUGIN =
        Boolean.getBoolean("forgeguard.patchGtoMixinPlugin");
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
        ...
        try {
            if (TRANSFORMATION_DECORATOR.equals(className)) {
                System.out.println("[Forgeguard] Patching TransformationServiceDecorator.onLoad");
                return patchTransformationServiceDecorator(buf);
            }
            if (LAUNCH_PLUGIN_HANDLER.equals(className)) {
                System.out.println("[Forgeguard] Patching LaunchPluginHandler plugin map guards");
            ...
            System.out.println("[Forgeguard] Failed to patch " + className + ": " + t);
        }
        return null;
    }
    ...
    private byte[] patchLegacyGtoMixinPlugin(String className, Class<?> classBeingRedefined, byte[] buf) {
        if (classBeingRedefined != null) {
            System.out.println("[Forgeguard] Legacy Phase 2: rewriting " + className.replace('/', '.'));
            return rewriteLegacyMixinMethods(buf);
        }

        System.out.println("[Forgeguard] Legacy Phase 1: " + className.replace('/', '.'));
            ...
            for (Class<?> cls : inst.getAllLoadedClasses()) {
                if (cls.getName().equals(className.replace('/', '.'))) {
                    try {
                        ...
                        System.out.println("[Forgeguard] Retransform failed: " + e);
                    }
                    return;
                }
            }
