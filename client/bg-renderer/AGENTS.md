...
- `BackgroundRenderer.kt`: public session API, render lifecycle, OpenGL scene passes, 4xMSAA framebuffer/resolve, readback, shaders, and performance monitor. Readback frames are published through the fixed three-slot `FrameSlotPool` in `BackgroundFramePainter.kt`; each slot owns one reusable Skia `Bitmap`/Compose `ImageBitmap` and uses FREE/WRITING/READY/DISPLAYED ownership plus draw-reader leases. The pool drops frames when all slots are reserved and never overwrites a displayed slot.
...
- `frame: StateFlow<Painter?>`
- `failure: StateFlow<String?>`
- `lowPerformance: StateFlow<Boolean>`
- `setActive(Boolean)`
- `close()`

The renderer uses a hidden OpenGL 4.5 core context on the daemon thread `rdi-background-renderer`. It renders a fixed `1280x720` frame at `20FPS`, synchronously reads BGRA pixels, vertically flips them into a reusable slot's Pixmap, and publishes a lightweight `BackgroundFramePainter` token. `AppBackgroundImage` draws the token directly; it must not wrap each frame in `BitmapPainter`.

The scene currently contains:

- A deterministic irregular three-depth harbor made from `HarborBlock` descriptors: `Foreground`, `Midground`, and `Background` terraces with per-layer drift/zigzag offsets, left/right docks and detailed shelters, `LeftPavilion`, `CenterMarket`, and the shifted `RightLighthouse` at x≈18; Phase2B adds 53 sand terrace layers (Foreground 9, Midground 16, Background 28) for foreground shoulders, midground banks, and side-peak/low-center background silhouette; the layout spans approximately `x=-60..60`
...
- A small oak-plank boat whose local kind-2 mesh is transformed by the animated pose centered at `BOAT_CENTER_X=-7`, `BOAT_CENTER_Z=25`
...
The camera uses a fixed distance of `50`, base height `8`, height swing `0.25`, target `(0, 5, -25)`, FOV `60`, and yaw sweep `0.045` radians. The boat uses `BOAT_VISUAL_SCALE=1.25` and the player uses `PLAYER_VISUAL_SCALE=0.08`, while the boat remains centered at `BOAT_CENTER_X=-7`, `BOAT_CENTER_Z=25`; this moves its view depth from about50 to about25 and approximately doubles its projected size without changing scale. The static scene mesh is initialized once into one VBO and one EBO with contiguous material index batches; the main, reflection, static shadow, and water passes reuse it through indexed draws. Static exposed quads store four shared-corner vertices and six indices; boat and water retain their legacy expanded vertex counts while also using the shared index stream. The pre-water scene is captured before the water pass, and only the boat's local geometry uses kind `2` model transformation.
...
The water shader does not upload or sample a Minecraft water texture. Its deep-blue stable base tint `(0.08, 0.30, 0.58)` and base opacity `0.65` come from the `WATER` material color and shader constants, while the Complementary water resources `assets/complementary/textures/cloud-water.png` (256x256) and `noise.png` (128x128) provide the animated normal and noise layers. Four normal layers use scales `1`, `4`, `0.25`, and `0.05`, weights `1.10`, `0.35`, `1.55`, and `0.85`, and view-edge amplitude `0.27*(1-0.60*viewEdge)`, making large-scale waves dominant. Near water receives the `(0.92,0.98,1.04)` tint over 18..110 units. Water absorption uses `exp(-vec3(0.22, 0.10, 0.035) * thickness)`, and the shader receives the shared noon sky color for fallback reflection. They use repeat wrapping, linear filtering, and generated mipmaps.
Each texture is decoded and uploaded once on the render thread; every handle is released there during renderer shutdown.
Water geometry retains its vertex wave and normal-layer animation without vanilla texture-frame animation. The CPU truth source for rendered water height is `waterVertexOffset` plus the exact two-triangle interpolation in `waterSurfaceHeightAt`; `WATER_VERTEX_SHADER` interpolates the same six `WATER_WAVE_*` constants, so its vertex displacement remains numerically aligned with CPU sampling. `BackgroundScene.frameState` computes one wrapped `waterTimeSeconds` for both the water draw and boat pose. The boat Y follows the sampled surface at its animated X and fixed Z, and `playerAnchor` continues to derive from that same boat pose with a `+0.05` Y offset.
The water quad remains a single six-vertex draw within the fixed water material batch. Its shader samples captured pre-water scene color/depth for depth-rejected refraction, Beer-Lambert water absorption, and shallow shoreline foam; it combines that transmission with the half-resolution mipmapped planar reflection using Schlick water Fresnel (F0=0.02), explicit distance/edge LOD clamped to3.5, reflection UV distortion `normalXY*0.012`, and reflection strength `clamp(0.06+fresnel*0.92,0.0,0.92)` for a clearer stronger reflection. The restrained GGX sun highlight retains roughness0.12 but is scaled by0.08 and capped at1.8, with the existing distance/edge solar fade over 80..180 units, then xz-distance horizon fog uses the shared water-distance calculation and outputs an already-composited opaque pixel (`alpha=1`); other materials are not fogged. The reflection contains the sky, fixed sun, non-water scene, player, and cached cloud mesh. Reflection geometry keeps the normal `GL_CCW` front-face winding: the ordinary `lookAt` reflection view has no negative-scale determinant. It still uses a water clip plane, and state is restored to CCW with clipping disabled before the main water pass. Cloud geometry is generated once from the main camera coordinates and reused with a reflection-eye height compensation.

Color and HDR contract:

- Authored scene colors remain the fixed noon sRGB values in `NOON_*_COLOR`; `SkyColor.toLinear()` uses the exact piecewise sRGB transfer function. Scene vertex tints, clear colors, water horizon fog, and cloud fog uniforms use explicitly converted linear values before HDR lighting and blending. Terrain/cloud gains, absorption, Fresnel/GGX, shadows, emission intensity, Bloom, and Tonemap constants remain linear/HDR coefficients.
- `TextureEncoding.SrgbColor` is used for block, lantern, skin, cape, sun, and cloud color PNGs; `TextureEncoding.LinearData` is used for cloud-water normal and noise maps. Do not pre-transform bytes or enable `GL_FRAMEBUFFER_SRGB`.
- The procedural noon sky function accepts authored sRGB parameters and returns display-like sky color. Sky dither is applied in that display space, then the result is converted exactly once to linear before writing HDR. Water fallback reflection uses the same display-space function without dither and converts it to linear before mixing.
- The first post-processing pass is an HDR linear copy into the mipmapped HDR texture, not a global `pow(2.2)` linearization. The final RGBA8 tonemap performs the sole linear-to-sRGB conversion.
- `BlockMaterial` owns explicit `alphaCutout`, `textureVScale`, and `emissive` traits. `ordinal` may index synchronized CPU material buckets/textures only; it is not a GLSL semantic protocol.
...
- The harbor layout must remain deterministic descriptor data in `HarborBlock`/`HarborGroup`, with irregular per-layer terrace offsets, `Foreground`, `Midground`, `Background`, left/right docks and detailed shelters, `LeftPavilion`, `CenterMarket`, and `RightLighthouse` (centered at x≈18) spanning approximately `x=-60..60`. Phase2B adds exactly 53 sand layers after the existing Background terraces and before pier descriptors: group counts are Foreground 19, Midground 38, Background 63; the protected `x=-24..24,z=-24..-8` corridor remains clear, narrow right framing stays within `24 < x <= 28`, and background side peaks top at `12.85`, above the lower center top at `10.35` and below the lighthouse. Seeded far dunes and fixed cacti remain separate data sources.
- The static scene mesh must be initialized once into one VBO and one EBO, with index batches reused by main, reflection, static shadow, and water passes. Keep the scene vertex count below `SCENE_VERTEX_BUDGET=14_000`; the boat is the only kind-2 geometry and is transformed from its local mesh by the animated boat pose centered at `BOAT_CENTER_X=-7`, `BOAT_CENTER_Z=25`.
...
- The fixed camera contract is distance `50`, base height `8`, height swing `0.25`, target `(0, 5, -25)`, FOV `60`, and yaw sweep `0.045` radians. Keep the UI-safe central crop around the boat/player focal point; the boat/player anchor is centered at `(-7, 25)` with unchanged visual scales.
- Each frame must follow `main HDR MSAA framebuffer bind -> pre-water scene render -> HDR color/depth capture into single-sample scene textures -> 640x360 HDR planar reflection pass -> main MSAA framebuffer rebind -> generate reflection mipmaps -> independent water render -> raw HDR resolve -> HDR linear copy -> generate mipmaps -> Bloom atlas -> Bloom/Tonemap/saturation -> final RGBA8 READ framebuffer -> BGRA glReadPixels`. Call `pixels.clear()` immediately before every `glReadPixels`.
- The MSAA framebuffer has 4x `GL_R11F_G11F_B10F` color/depth renderbuffers. The pre-water scene, reflection, and raw resolve use the same alpha-less HDR color format; depth remains DEPTH_COMPONENT24. The final post-process target is `GL_RGBA8`. Release all OpenGL framebuffers, renderbuffers, textures, programs, VAO, VBO, and scene EBO on the render thread.
- Frame publication uses exactly three reusable Skia `Bitmap`/Compose `ImageBitmap` pairs per session. Each pair is created once through the atomic `SkiaFrameSlot` factory; no frame may call `makeRaster`, `toComposeImageBitmap`, or allocate another width×height×4 pixel object. A `DISPLAYED` slot remains reserved across draw calls, and its reader lease prevents close or reuse until the final draw releases it.
...
The frame-pool ownership and renderer thread publication contracts are covered by `BackgroundFramePoolTest`: exactly three reusable payload slots, READY replacement, DISPLAYED reader leases, bounded frame drops, close deferral, and deterministic ensure/close interleaving. The pool is CPU-only in tests; Skia/OpenGL frame production still requires the desktop client.

On 2026-08-30, the required Windows validation command from `client/ui` passed: `:render-core:test`, `:bg-renderer:test` (67 tests, including water displacement/interpolation, boat-following, and shader constant contracts), `:player-model:test`, and `:compileKotlin`. Kotlin compiler-daemon connection retries were reported, but Gradle's documented fallback compiler completed successfully. Real GPU rendering and long-running native-memory validation remain pending; the boat/water change has not had real GPU visual confirmation.
...
- Phase2A exposed-surface implementation adds deterministic axis-aligned rectangle subtraction in `StaticSurface.kt`; static mesh integration preserves the complete `staticAabbs` list for AO/shadow framing and keeps boat/water geometry unchanged. Indexed scene storage measured exactly 10,390 vertices and 15,510 indices for production seed `123L` before Phase2B (static 10,240 vertices/15,360 indices, boat+water 150/150), below both the former 13,182 expanded vertex count and the 14,000 budget. Phase2B adds 53 deterministic sand layers and the final indexed scene measures exactly 11,690 vertices and 17,460 indices (static 11,540 vertices/17,310 indices, boat+water 150/150), below the 12,500 Phase2B target and 14,000 budget. The boat/player anchor is now `(-7,25)` with unchanged visual scale, and projection tests verify approximately 2x legacy view-depth ratio across the camera track. Reflection water detail now uses per-frame mipmap generation and explicit shader LOD/normal tuning. The command `pwsh.exe -NoLogo -NoProfile -Command "Set-Location 'C:\\Users\\calebxzhou\\Documents\\coding\\rdi5\\client\\ui'; .\\gradlew.bat :bg-renderer:test :bg-renderer:compileKotlin --rerun-tasks --no-daemon"` passed with 52 tests (Scene27, Shader17, StaticSurface7, Resource1). Focused `StaticSurfaceTest` covers winding, adjacent/partial/duplicate/coplanar occlusion, deterministic ordering, and UV continuity. Real GPU validation remains pending for EBO/VAO correctness, epsilon seam behavior, R11F_G11F_B10F NPOT reflection mipmaps/LOD, visual water quality, and the extra per-frame mipmap cost; the current indexed scene has 5,820 triangles, which may affect the 45ms low-performance policy.
...
- Reflection uses a 640x360 mipmapped HDR texture regenerated once per frame before water sampling; this reduces distant/blocky reflection detail at the cost of extra mipmap generation work counted by the existing 45ms policy.
...
- Static AABBs use deterministic exposed-face rectangle subtraction, so hidden internal and duplicate surfaces are omitted while the complete AABB list remains available for AO and shadow framing. Same-source adjacent fragments are coalesced; different source/material external surfaces are not merged.
