...
- Each frame must follow `main HDR MSAA framebuffer bind -> pre-water scene render -> HDR color/depth capture into single-sample scene textures -> 640x360 HDR planar reflection pass -> main MSAA framebuffer rebind -> generate reflection mipmaps -> independent water render -> raw HDR resolve -> linearize -> generate mipmaps -> Bloom atlas -> Bloom/Tonemap/saturation -> final RGBA8 READ framebuffer -> BGRA glReadPixels`. Call `pixels.clear()` immediately before every `glReadPixels`.
