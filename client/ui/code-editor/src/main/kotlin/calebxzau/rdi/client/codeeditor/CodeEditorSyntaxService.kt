...
private val punctuationStyle = CodeTokenRole.PUNCTUATION
private val jsonKeyStyle = CodeTokenRole.JSON_KEY
private val jsonStringStyle = CodeTokenRole.JSON_STRING
private val jsonNumberStyle = CodeTokenRole.JSON_NUMBER
private val jsonLiteralStyle = CodeTokenRole.JSON_LITERAL
private val tomlTableStyle = CodeTokenRole.TOML_TABLE
private val tomlKeyStyle = CodeTokenRole.TOML_KEY
private val tomlStringStyle = CodeTokenRole.TOML_STRING
private val tomlNumberStyle = CodeTokenRole.TOML_NUMBER
private val tomlLiteralStyle = CodeTokenRole.TOML_LITERAL
private val tomlCommentStyle = CodeTokenRole.TOML_COMMENT
private val yamlKeyStyle = CodeTokenRole.YAML_KEY
private val yamlStringStyle = CodeTokenRole.YAML_STRING
private val yamlNumberStyle = CodeTokenRole.YAML_NUMBER
private val yamlLiteralStyle = CodeTokenRole.YAML_LITERAL
private val yamlCommentStyle = CodeTokenRole.YAML_COMMENT
private val yamlAnchorStyle = CodeTokenRole.YAML_ANCHOR
private val cfgSectionStyle = CodeTokenRole.CFG_SECTION
private val cfgTypeStyle = CodeTokenRole.CFG_TYPE
private val cfgKeyStyle = CodeTokenRole.CFG_KEY
private val cfgValueStyle = CodeTokenRole.CFG_VALUE
private val cfgNumberStyle = CodeTokenRole.CFG_NUMBER
private val cfgCommentStyle = CodeTokenRole.CFG_COMMENT
...
private class HighlightCollector {
    val spans = mutableListOf<CodeHighlightSpan>()

    fun addSpan(role: CodeTokenRole, start: Int, end: Int) {
        if (start in 0 until end) {
            spans += CodeHighlightSpan(role, start, end)
        }
    }
}

fun highlightCode(text: String, language: CodeLanguage): List<CodeHighlightSpan> {
    val collector = HighlightCollector()
    when (language) {
        CodeLanguage.JSON,
        CodeLanguage.JSON5 -> collector.applyJsonHighlight(text)
        CodeLanguage.TOML -> collector.applyTomlHighlight(text)
        CodeLanguage.YAML -> collector.applyYamlHighlight(text)
        CodeLanguage.FORGE_CFG -> collector.applyForgeCfgHighlight(text)
        CodeLanguage.PLAIN_TEXT -> Unit
    }
    return collector.spans.toList()
}
...
private fun HighlightCollector.applyJsonHighlight(text: String) {
    var index = 0
...
private fun HighlightCollector.applyTomlHighlight(text: String) {
    var lineStart = 0
...
private fun HighlightCollector.applyTomlLineHighlight(text: String, start: Int, end: Int) {
    if (start >= end) return
...
private fun HighlightCollector.applyTomlValueHighlight(text: String, start: Int, end: Int) {
    var index = start
    while (index < end) {
        when (text[index]) {
            '"', '\'' -> {
...
private fun HighlightCollector.applyYamlHighlight(text: String) {
    var lineStart = 0
    while (lineStart <= text.length) {
        val lineEnd = text.indexOf('\n', lineStart).let { if (it == -1) text.length else it }
...
private fun HighlightCollector.applyYamlLineHighlight(text: String, start: Int, end: Int) {
    if (start >= end) return
...
private fun HighlightCollector.applyYamlKeyHighlight(text: String, start: Int, end: Int) {
...
private fun HighlightCollector.applyYamlValueHighlight(text: String, start: Int, end: Int) {
...
private fun HighlightCollector.applyYamlTokenHighlight(text: String, start: Int, end: Int) {
    if (start >= end) return
...
private fun HighlightCollector.applyForgeCfgHighlight(text: String) {
    var lineStart = 0
    while (lineStart <= text.length) {
        val lineEnd = text.indexOf('\n', lineStart).let { if (it == -1) text.length else it }
...
private fun HighlightCollector.applyForgeCfgLineHighlight(text: String, start: Int, end: Int) {
    if (start >= end) return
...
private fun HighlightCollector.applyForgeCfgValueHighlight(text: String, start: Int, end: Int) {
    var index = skipWhitespace(text, start, end)
    while (index < end) {
        when (text[index]) {
            '<', '>', '{', '}', ':' -> {
...
private fun HighlightCollector.safeAddStyle(style: CodeTokenRole, start: Int, end: Int) {
