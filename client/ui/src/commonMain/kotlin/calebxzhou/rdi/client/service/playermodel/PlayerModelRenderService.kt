...
object PlayerModelRenderService {
    fun buildSkinData(image: ImageBitmap?): SkinData? = image?.let(::SkinData)
    ...
    ): PlayerModelRenderer? = skin?.let { PlayerModelRenderer(it, cape, slim, showOuterLayer) }

    fun renderArgb(
        renderer: PlayerModelRenderer,
        taaState: TemporalAaState,
        width: Int,
        height: Int,
        orbitYawDeg: Float,
        orbitPitchDeg: Float,
        walkPhaseDeg: Float,
        enableTaa: Boolean,
        taaLowResThreshold: Int
    ): IntArray {
        val taaEnabledNow = enableTaa && min(width, height) <= max(64, taaLowResThreshold)
...
class SkinData(image: ImageBitmap) {
...
internal const val DEFAULT_ORBIT_YAW_DEG = 180f

class TemporalAaState {
