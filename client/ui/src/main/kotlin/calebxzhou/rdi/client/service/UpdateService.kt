...
object UpdateService {
    suspend fun startUpdateFlow(
        ...
        runCatching {
            ...
            val updateSync = syncUpdates(onStatus, onDetail)
            if (!updateSync.success) {
                onStatus("更新失败，请检查网络")
                return@runCatching
            }
            if (updateSync.requiresRestart) {
                onStatus("更新完成，需要重启")
                onRestart?.invoke()
                return@runCatching
            }
...
typealias UpdateSyncResult = Pair<Boolean, Boolean>

private val UpdateSyncResult.success: Boolean
    get() = first

private val UpdateSyncResult.requiresRestart: Boolean
    get() = second
