...
static void dbg(const char* fmt, ...) {
    ...
    const DWORD tempPathLength = GetTempPathW(static_cast<DWORD>(std::size(tempPath)), tempPath);
    if (tempPathLength == 0 || tempPathLength >= std::size(tempPath)) {
        return;
    }
    const auto debugDir = std::filesystem::path(tempPath) / L"rdi-webview2";
    std::error_code error;
    std::filesystem::create_directories(debugDir, error);
    if (error) {
        return;
    }
    FILE* f = _wfopen((debugDir / L"debug.txt").c_str(), L"a");
    if (!f) return;
    va_list args;
    va_start(args, fmt);
    vfprintf(f, fmt, args);
