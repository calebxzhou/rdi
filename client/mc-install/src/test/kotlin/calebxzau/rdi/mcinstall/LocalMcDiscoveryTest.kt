...
class LocalMcDiscoveryTest {
    ...
    fun `valid stored rows update last seen and missing rows are deleted`() = kotlinx.coroutines.runBlocking {
        ...
        val validator = MapValidator(
            stored = mapOf(valid to McInstallationValidation.Valid(valid))
        )
    ...
    fun `inaccessible stored rows remain unchanged`() = kotlinx.coroutines.runBlocking {
        ...
        val validator = MapValidator(
            stored = mapOf(inaccessible to McInstallationValidation.Inaccessible)
        )
    ...
    fun `known APPDATA and USERPROFILE candidates are checked without RDI fallback`() = kotlinx.coroutines.runBlocking {
        ...
        val validator = MapValidator(
            discovered = mapOf(
                appDataCandidate to McInstallationValidation.Valid(appDataCandidate),
                userProfileCandidate to McInstallationValidation.Valid(userProfileCandidate)
            )
        )
    ...
    private suspend fun runService(
        ...
        clock: FixedClock,
        scanner: McInstallationScanner = fakeScanner { _, _ -> Result.success(emptyList()) },
