...
class LocalMcArtifactFileReuser(
    ...
    private fun matches(path: Path, request: LocalArtifactRequest): Boolean {
        ...
        val updated = when (request.algorithm) {
            ...
            LocalArtifactHashAlgorithm.SHA256 -> cached.sha256?.let { cached }
                ?: cached.copy(sha256 = path.toFile().sha256.lowercase())
        ...
        return when (request.algorithm) {
            ...
            LocalArtifactHashAlgorithm.SHA256 -> updated.sha256.equals(request.hash.trim(), ignoreCase = true)
