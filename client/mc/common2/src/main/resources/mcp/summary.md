...
- Need the player's inventory, hotbar, armor, offhand, or empty slots: call `/inventory`; it returns CSV by default and uses `minecraft:air,count=0` for empty slots. Use `/inventory/detail?section=...&slot=...` only when exact raw `snbt` for one slot is needed.
...
- Need to craft from chosen inventory slots: call `POST /craft`; it does not need a crafting table, open GUI, or recipe book state. Use `POST /craft/parallel` for independent parallel batches such as several small-stone groups into larger stones; entries in the same parallel batch cannot use another entry's output.
...
- Need to move the player to a nearby position in the current dimension: call `POST /move` with JSON `{"x":...,"y":...,"z":...}` or query `?x=...&y=...&z=...`; `/move` searches within 4 blocks of that point for a safe standable target and returns an error if none exists.
...
Position based APIs always read the current loaded dimension. Pass only `x/y/z` coordinates and do not include a dimension ID.
...
- Call `/inventory` when exact item stacks or slots are needed; parse its CSV rows. Do not use `/player?detail=true` only for inventory.
...
- `POST /craft` runs server-side from selected inventory slots; it does not require a nearby crafting table, an open menu, or recipe book state. `POST /craft/parallel` is for independent craft entries planned from the same starting inventory; it is not a sequential crafting chain.
...
- To craft an item, resolve display names with `/item-search?text=...`, then call compact `/recipe?itemId=...` and `/inventory`, choose source inventory slots, build a `shape`, then call `POST /craft` with `dryRun=true` before the real action when the slot plan is uncertain. Use `/craft/parallel` only when each entry is independent from the others.
- `/recipe`, `/entity`, `/blockentity`, `/container`, `/quest/detail/{id}`, and `/nearby-resources` return compact summaries by default. Use summary fields first; open Markdown `detail` for one relevant entry when compact data is not enough. Use `detailJson` only when exact structured fields are needed. Use `view=full` only for debugging or extraction, not normal planning.
...
- For terrain or build understanding, use `/blockmap/slice` for horizontal layout and `POST /blockstate/batch` for exact cells.
- Use `POST /blockstate/batch` for exact cells unless there is only one cell.
...
- `GET /inventory/detail?section=hotbar&slot=0`: `GET /apidoc/inventory.md`
...
- `POST /craft/parallel`: `GET /apidoc/craft$parallel.md`
...
- `GET /blockentity?pos=x,y,z`: `GET /apidoc/blockentity.md`
- `GET /blockentity/detail?ref=x,y,z`: `GET /apidoc/blockentity.md`
- `GET /blockentity/detail.md?ref=x,y,z`: `GET /apidoc/blockentity.md`
- `GET /blockentity/detail.json?ref=x,y,z`: `GET /apidoc/blockentity.md`
- `GET /sign/text?x=10&y=64&z=-20`: `GET /apidoc/sign$text$get.md`
- `POST /sign/text`: `GET /apidoc/sign$text.md`
- `GET /container?pos=x,y,z&side=north`: `GET /apidoc/container.md`
- `GET /container/detail?ref=x,y,z~side`: `GET /apidoc/container.md`
- `GET /container/detail.md?ref=x,y,z~side`: `GET /apidoc/container.md`
- `GET /container/detail.json?ref=x,y,z~side`: `GET /apidoc/container.md`
