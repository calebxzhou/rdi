...
Default response is compact and omits raw SNBT. Use `GET /blockentity/detail?ref=dim,x,y,z` for Markdown, or `GET /blockentity/detail.json?ref=dim,x,y,z` for exact JSON/SNBT. `.detail.md` is a compatibility alias for Markdown. Use `view=full` only for debugging or extraction.
