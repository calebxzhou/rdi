...
object JeiRecipeProcessCollector {
    ...
    private fun Any.minecraftRecipe(): Recipe<*>? {
        return (this as? RecipeHolder<*>)?.value() as? Recipe<*>
    }
        ...
        if (!isCustom) {
            for (value in values) {
                when (value) {
                    ...
                    is Ingredient.ItemValue -> recipeStack(value.item())?.let { items += it }
                }
            }
        }
        if (items.isEmpty() && tags.isEmpty()) {
            for (stack in this.items) {
                recipeStack(stack)?.let { items += it }
            }
        }
        val distinctItems = items.distinctBy { it.itemId to it.count }
        if (tags.isEmpty()) {
            distinctItems.tagIngredient()?.let { return it }
        }
        return RecipeIngredient(
            items = distinctItems,
            tags = tags.distinctBy { it.tagId },
        )
    }
    ...
    private fun itemTag(tag: net.minecraft.tags.TagKey<net.minecraft.world.item.Item>): RecipeItemTag {
        ...
        for (holder in BuiltInRegistries.ITEM.getTagOrEmpty(tag)) {
            ...
            if (examples.size < MAX_TAG_EXAMPLES) {
                recipeStack(ItemStack(holder))?.let { examples += it }
            }
        }
    ...
    private fun CapturedSlot.ingredient(): RecipeIngredient? {
        ...
        minecraftIngredient?.let { ingredient ->
            ...
            ingredient.items.tagIngredient()?.let { return it.copy(fluids = fluids.distinctBy { fluid -> fluid.fluidId to fluid.amount }) }
            return ingredient
        }
        if (items.isEmpty() && fluids.isEmpty()) return null
        items.tagIngredient()?.let { return it.copy(fluids = fluids.distinctBy { fluid -> fluid.fluidId to fluid.amount }) }
        return RecipeIngredient(
    ...
    private class CapturingRecipeSlotBuilder(
        ...
        override fun addFluidStack(fluid: Fluid, amount: Long, component: DataComponentPatch): IRecipeSlotBuilder {
