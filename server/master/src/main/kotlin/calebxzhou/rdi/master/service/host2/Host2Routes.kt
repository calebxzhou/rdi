...
fun Route.host2Routes() {
    ...
    val modsService by inject<Host2ModsService>()
    val fileService by inject<Host2FileService>()

    route("/host2") {
        post {
            response(data = hostService.create(call.player(), call.receive()))
        }
        get("/my") {
            response(data = hostService.list(call.player(), true, call.page()))
        }
        get("/list") {
        ...
        route("/{id}") {
            ...
            put("/server-pack") {
                setupService.receive(call.player(), call.host2Id(), call)
                ok("server pack已接收，正在处理")
            }
            delete("/server-pack") {
                setupService.deleteServerPack(call.player(), call.host2Id())
                ok()
            }
            put("/ownership") {
            ...
            post("/start") {
                ...
                hostService.withMutation(id) {
                    Host2RuntimeService.start(hostService.requireCanStart(call.player(), id))
                }
                ok()
            }
            post("/stop") {
                val id = call.host2Id()
                hostService.withMutation(id) {
                    Host2RuntimeService.stop(hostService.requireAdmin(call.player(), id).id)
                }
                ok()
            }
            post("/restart") {
                val id = call.host2Id()
                hostService.withMutation(id) {
                    Host2RuntimeService.restart(hostService.requireAdmin(call.player(), id))
                }
                ok()
            }
            post("/command") {
            ...
            route("/mods") {
                get {
                    response(data = modsService.list(call.player(), call.host2Id()))
                }
                post {
                    modsService.add(call.player(), call.host2Id(), call.receive<List<Mod>>())
                    ok()
                }
                delete {
                    modsService.delete(call.player(), call.host2Id(), call.receive<List<Host2.ModKey>>())
                    ok()
                }
                put("/enabled") {
                    modsService.setEnabled(call.player(), call.host2Id(), call.receive())
                    ok()
                }
            }
            route("/log") {
