...
public final class RcmdServerCommands {
    ...
    private static RcmdResult handleTpa(RcmdContext context) {
        EntityPlayerMP requester = playerOrNull(context.source());
        if (requester == null) {
            return RcmdResult.error("此rcmd命令只能由玩家执行");
        }
        var result = TpaService.request(
                new TpaPlayer112(requester),
                context.getString("playerName"),
                new TpaPlayerLookup112(requester.server)
        );
        return toRcmdResult(result);
    }

    private static RcmdResult handleTpok(RcmdContext context) {
        EntityPlayerMP target = playerOrNull(context.source());
        if (target == null) {
            return RcmdResult.error("此rcmd命令只能由玩家执行");
        }
        var result = TpaService.accept(new TpaPlayer112(target), new TpaPlayerLookup112(target.server));
        return toRcmdResult(result);
    }

    private static RcmdResult handleSetHome(RcmdContext context) {
        EntityPlayerMP player = playerOrNull(context.source());
        if (player == null) {
            return RcmdResult.error("此rcmd命令只能由玩家执行");
        }
        return toRcmdResult(HomeService.setHome(new HomePlayer112(player), context.getString("name")));
    }
    ...
    private static RcmdResult handleHome(RcmdContext context) {
        ...
        return toRcmdResult(HomeService.goHome(new HomePlayer112(player), context.getString("name")));
    }
    ...
    private static RcmdResult handleListHome(RcmdContext context) {
        ...
        return toRcmdResult(HomeService.listHomes(new HomePlayer112(player)));
    }
    ...
    private static RcmdResult handleDeleteHome(RcmdContext context) {
        ...
        return toRcmdResult(HomeService.deleteHome(new HomePlayer112(player), context.getString("name")));
    }
    ...
    private static EntityPlayerMP playerOrNull(RcmdSource source) {
        if (source instanceof RcmdServerSource112 playerSource) {
            return playerSource.getPlayer();
        }
        if (source instanceof RcmdCommandSenderSource112 commandSource) {
