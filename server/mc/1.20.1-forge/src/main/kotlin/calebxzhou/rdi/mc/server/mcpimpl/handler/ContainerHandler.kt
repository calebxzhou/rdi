...
import calebxzhou.rdi.mc.server.mcpimpl.handler.BlockHandler.mcBpos
...
object ContainerHandler {
    ...
    private fun readContainer(player: ServerPlayer, pos: RBlockPos): ActionResult<ContainerSlotListP.Group> {
        ...
        val handler = itemHandler(player, mcpos) ?: return ActionResult.Err("not a container")
        val slots = (0 until handler.slots).map { slotId -> slot(slotId, handler.getStackInSlot(slotId)) }
        return ActionResult.Ok(ContainerSlotListP.Group(pos, state.block.resId.toString(), slots))
    }
    ...
    private fun resolveContainer(ref: ContainerRef, player: ServerPlayer): ActionResult<ResolvedContainer> {
        ...
        val handler = itemHandler(player, blockPos) ?: return ActionResult.Err("not a container")
        return ActionResult.Ok(ResolvedContainer("block:$pos", handler, blockPos))
    }

    private fun itemHandler(player: ServerPlayer, blockPos: BlockPos): IItemHandler? {
        val blockEntity = player.serverLevel().getBlockEntity(blockPos) ?: return null
        return blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, null).orElse(null)
    }

    private fun moveOne(from: ResolvedContainer, to: ResolvedContainer, move: ContainerMoveQ.Move, test: Boolean, player: ServerPlayer): ActionResult<Int> {
    ...
    private fun insertionPlan(target: IItemHandler, targetSlotId: Int?, stack: ItemStack, excludedSlotId: Int?): InsertPlan {
    ...
    private fun IItemHandler.autoTargetSlotIds(stack: ItemStack, excludedSlotId: Int?): List<Int> {
        ...
        val mergeSlots = slotIds.filter { slotId ->
            ...
            !targetStack.isEmpty && ItemStack.isSameItemSameTags(targetStack, stack)
        }
    ...
    private fun IItemHandler.validSlot(slotId: Int): Boolean = slotId in 0 until slots

    private data class ResolvedContainer(val key: String, val handler: IItemHandler, val blockPos: BlockPos?)
    private data class InsertStep(val slotId: Int, val count: Int)
    private data class InsertPlan(val count: Int, val steps: List<InsertStep>)

    private val Item.resId: ResourceLocation get() = BuiltInRegistries.ITEM.getKey(this)
    private val net.minecraft.world.level.block.Block.resId: ResourceLocation get() = BuiltInRegistries.BLOCK.getKey(this)
}
